...
from ..utils import (
    check_admin_permissions, refresh_token, get_guild_details,
    get_db_async, run_async, fetch_user_details_http, GUILDS_URL
)
...
def dashboard_home():
    ...
    try:
        ...
        shared_guilds = sorted([g for g in admin_guilds if int(g['id']) in bot_guild_ids], key=lambda g: g['name'].lower())
        owned_servers = [g for g in shared_guilds if g['owner']]
        admin_servers = [g for g in shared_guilds if not g['owner']]
        
        return render_template('index.html', owned_servers=owned_servers, admin_servers=admin_servers, user_info=user_info, logged_in=True, bot_guilds_count=len(bot.guilds), bot_users_count=sum(g.member_count for g in bot.guilds))
...
def server_home(server_id):
    ...
    return render_template('dashboard.html', server=target_guild, guild_details=guild_details, monthly_warnings=monthly_warnings_count, top_member=top_member_details, recent_logs=enriched_logs)
...
def announcement(server_id):
    ...
    if request.method == 'POST':
        ...
        if run_async(send_announcement_async()):
            ...
            flash("Erreur : Impossible de trouver le salon ou d'y envoyer le message.", "danger")
        return redirect(url_for('dashboard.announcement', server_id=server_id))
...
# ... Ajoutez ici les autres routes du dashboard (messagelogs, etc.) sur le même modèle ...