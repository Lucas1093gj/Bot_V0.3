...
from ..utils import get_db_async, run_async, is_bot_admin, get_guild_details
...
def dashboard():
    ...
    def get_admin_stats():
        async def _get_stats():
            ...
            active_guilds = []
            for g in active_guilds_data:
                guild_id_str = str(g['guild_id'])
                guild = bot.get_guild(int(guild_id_str))
                guild_name = guild.name if guild else get_guild_details(guild_id_str).get('name', f"Serveur Inconnu ({guild_id_str})")
                active_guilds.append({'name': guild_name, 'count': g['count']})
...
def restart():
    def restart_script():
        # Utilise os.execv pour remplacer le processus actuel par un nouveau
