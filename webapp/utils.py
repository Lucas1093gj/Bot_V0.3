...
def is_valid_url(url):
    """
    Vérifie si une URL est bien formée, commence par http/https, et se termine par une extension d'image commune.
    """
    if not url or len(url) > 2048:
        return False
    
    # Vérifie la structure générale de l'URL et qu'elle commence par http/https
    basic_check = re.match(r'^https?://[^\s/$.?#].[^\s]*$', url)
    # Vérifie si l'URL (ignorant les paramètres après '?') se termine par une extension d'image
    image_extension_check = re.search(r'\.(jpg|jpeg|png|gif|webp)(\?.*)?$', url, re.IGNORECASE)
    
    return basic_check is not None and image_extension_check is not None
...
def register_template_filters(app, translator):
    """Enregistre les filtres et fonctions personnalisés pour Jinja2."""
    @app.context_processor
    def inject_utils():
        """Injecte des fonctions et des variables dans le contexte de tous les templates."""
        return dict(
            # is_bot_admin est déjà injecté globalement dans create_app
            _=translator.get_text,  # Rend la fonction de traduction `_` disponible dans les templates
